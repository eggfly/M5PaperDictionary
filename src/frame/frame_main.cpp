...
Frame_Main::Frame_Main(void) : Frame_Base(false)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        _key[i + 8] = new EPDGUI_Button("TEST", 20 + i * 136, 240 + 150, KEY_W, KEY_H);
    }
...
{
    if (!_names->isRenderExist(22))
    {
        _names->createRender(22, 26);
    }
    _names->setTextSize(22);
    _names->fillCanvas(0);
    ...
    _names->pushCanvas(0, 186 + 2 * 151, mode);
}
